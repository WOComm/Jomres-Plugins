...
class j03200stripe
{
	function __construct($componentArgs)
	{
		...
		if (!isset($thisJRUser->params['stripe']['customer']['id'] )) {
			...
			if ( isset($tmpBookingHandler->_tmpbooking['stripe']['customer_id'])) {
				$thisJRUser->params['stripe']['customer']['id'] = $tmpBookingHandler->_tmpbooking['stripe']['customer_id'];
		...
		$line_items = array('tax_code_id' => 0,
			...
			'transaction_id' => get_showtime("payment_intent")
		);
