...
class j06002editCoupon {
	function __construct()
		...
		if ($id > 0 && $jrportal_coupons->get_coupon()) {
			...
			$output['ISPERCENTAGE'] = jomresHTML::selectList($yesno, 'is_percentage', '', 'value', 'text', $jrportal_coupons->is_percentage);
			...
			$output['AMOUNT'] = 10.00;
			$output['ISPERCENTAGE'] = jomresHTML::selectList($yesno, 'is_percentage', '', 'value', 'text', 1);
		...
		//TODO
		$query = "SELECT `guests_uid`,`enc_firstname`,`enc_surname` FROM #__jomres_guests WHERE `property_uid` = ".(int)$defaultProperty;
		...
		if (!empty($result))
			...
			foreach ($result as $r)
				{
				$guests_arrray[$r->guests_uid] = array ("surname"=>$this->jomres_encryption->decrypt($r->enc_surname), "firstname"=>$this->jomres_encryption->decrypt($r->enc_firstname));
				}
			}
		...
		if (!empty($guests_arrray))
			...
			$output['GUEST_DROPDOWN'] = jomresHTML::selectList($guests_options, 'guest_uid', '', 'value', 'text', $guest_id,false);
			}
