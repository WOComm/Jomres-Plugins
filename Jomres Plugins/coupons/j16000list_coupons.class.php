...
class j16000list_coupons
	...
	function __construct()
		...
		$basic_coupon_details->get_all_coupons(array(0));
		
		/* if (isset($basic_coupon_details->coupons[0])) {
			foreach ($basic_coupon_details->coupons[0] as $k=>$v) {
		...
		if (!empty($guest_uids)) {
			...
			if (!empty($result)) {
				foreach ($result as $r) {
					$guests_array[$r->guests_uid] = array ("surname"=>$r->surname, "firstname"=>$r->firstname);
				}
			}
		} */
		
		
		foreach ($basic_coupon_details->coupons[0] as $coupon) {
			$r=array();
			...
			if (!using_bootstrap())
				...
				$jrtb .= $jrtbar->toolbarItem('edit',jomresURL(JOMRES_SITEPAGE_URL_ADMIN."&task=edit_coupon&coupon_id=".$coupon['id']),'');
				//$jrtb .= $jrtbar->toolbarItem('printer',jomresURL(JOMRES_SITEPAGE_URL_ADMIN."&task=print_coupons&tmpl=".get_showtime("tmplcomponent")."&popup=1&coupon_id=".$coupon['id']),'');
				$jrtb .= $jrtbar->toolbarItem('delete',jomresURL(JOMRES_SITEPAGE_URL_ADMIN."&task=delete_coupon&coupon_id=".$coupon['id']),'');
				...
				{
				$toolbar->newToolbar();
				$toolbar->addItem( 'icon-edit', 'btn btn-info', '', jomresURL( JOMRES_SITEPAGE_URL_ADMIN . '&task=edit_coupon' . '&coupon_id=' . $coupon['id'] ), jr_gettext( 'COMMON_EDIT', 'COMMON_EDIT', false ) );
				//$toolbar->addSecondaryItem( 'icon-print', '', '', jomresURL( JOMRES_SITEPAGE_URL_ADMIN . '&task=print_coupons&tmpl='.get_showtime("tmplcomponent").'&popup=1' . '&coupon_id=' . $coupon['id'] ), jr_gettext( 'JOMRES_COUPONS_PRINT_COUPONS', 'JOMRES_COUPONS_PRINT_COUPONS', false ) );
				$toolbar->addSecondaryItem( 'icon-trash', '', '', jomresURL( JOMRES_SITEPAGE_URL_ADMIN . '&task=delete_coupon' . '&coupon_id=' . $coupon['id'] ), jr_gettext( 'COMMON_DELETE', 'COMMON_DELETE', false ) );
		...
		$output['INFO_ADMIN']=jr_gettext('_JRPORTAL_COUPONS_DESC_ADMIN','_JRPORTAL_COUPONS_DESC_ADMIN',false);
		...
		$jrtbar =jomres_getSingleton('jomres_toolbar');
		$jrtb  = $jrtbar->startTable();
		$jrtb .= $jrtbar->toolbarItem('new',jomresURL(JOMRES_SITEPAGE_URL_ADMIN."&task=edit_coupon"),'');
		...
		$tmpl->readTemplatesFromInput( 'admin_list_coupons.html' );
