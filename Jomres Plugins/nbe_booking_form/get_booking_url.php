...
function get_booking_url($property_uid = 0, $type = 'sef', $params = '')
{
	...
	if ($mrConfig['tariffmode'] == '5' && !get_showtime('is_jintour_property')){
		$url = jomresURL(JOMRES_SITEPAGE_URL_NOSEF . '&task=nbe_booking_form&property_uid=' . $property_uid . '&language=' . get_showtime('lang') . $params);
		return $url;
	}
