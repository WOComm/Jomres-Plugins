...
class j06001overview_residents_ajax
	{
	function __construct()
		 ...
		 * Prefilter
		 */
		$clause = "WHERE a.property_uid = '".(int)$defaultProperty."' AND a.tag IS NOT NULL AND a.cancelled = '0' AND a.booked_in = '1' AND a.bookedout = '0' ";
		
		//date interval filter
		//$clause .= "AND DATE_FORMAT(a.arrival, '%Y/%m/%d') <= DATE_FORMAT('" . $today . "', '%Y/%m/%d') AND DATE_FORMAT(a.departure, '%Y/%m/%d') >= DATE_FORMAT('" . $today . "', '%Y/%m/%d') ";
