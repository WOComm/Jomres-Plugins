...
class RSSCreator091 extends FeedCreator {
	...
	function createFeed() {
		...
		$feed.= "    <channel>\n";
		$feed.= "        <title>".$this->iTrunc(htmlspecialchars($this->title),100)."</title>\n";
		...
		if ($this->image!=null) {
			$feed.= "        <image>\n";
			$feed.= "            <url>".$this->image->url."</url>\n";
			$feed.= "            <title>".$this->iTrunc(htmlspecialchars($this->image->title),100)."</title>\n";
			$feed.= "            <link>".$this->image->link."</link>\n";
			if ($this->image->width!="") {
		...
		if ($this->copyright!="") {
			$feed.= "        <copyright>".$this->iTrunc(htmlspecialchars($this->copyright),100)."</copyright>\n";
		}
		if ($this->editor!="") {
			$feed.= "        <managingEditor>".$this->iTrunc(htmlspecialchars($this->editor),100)."</managingEditor>\n";
		}
		if ($this->webmaster!="") {
			$feed.= "        <webMaster>".$this->iTrunc(htmlspecialchars($this->webmaster),100)."</webMaster>\n";
		}
		if ($this->pubDate!="") {
		...
		if ($this->docs!="") {
			$feed.= "        <docs>".$this->iTrunc(htmlspecialchars($this->docs),500)."</docs>\n";
		}
		if ($this->ttl!="") {
			$feed.= "        <ttl>".htmlspecialchars($this->ttl)."</ttl>\n";
		}
		if ($this->rating!="") {
			$feed.= "        <rating>".$this->iTrunc(htmlspecialchars($this->rating),500)."</rating>\n";
		}
		if ($this->skipHours!="") {
		...
		for ($i=0,$n=count($this->items);$i<$n;$i++) {
			$feed.= "        <item>\n";
			$feed.= "            <title>".$this->iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)),100)."</title>\n";
			$feed.= "            <link>".htmlspecialchars($this->items[$i]->link)."</link>\n";
 ...
 */
 class AtomCreator10 extends FeedCreator {
	...
	function createFeed() {
		...
		if ($this->copyright!="") {
					$feed.= "        <rights>".$this->iTrunc(htmlspecialchars($this->copyright),100)."</rights>\n";
		}
		$feed.= "    <generator>".$this->version()."</generator>\n";
...
class MBOXCreator extends FeedCreator {
	...
	function createFeed() {
		for ($i=0,$n=count($this->items);$i<$n;$i++) {
			...
			$feed.= "Subject: ".MBOXCreator::qp_enc($this->iTrunc($this->items[$i]->title,100))."\n";
			$feed.= "\n";
	...
	function createFeed() {
		...
		if ($this->image!=null) {
			$imageStr = "<a href='".$this->image->link."'".$targetInsert.">".
							...
							$this->iTrunc(htmlspecialchars($this->image->title),100).
		...
		if ($this->title) {
			$feedArray[] = "<div class='".$this->stylePrefix."title'><a href='".$this->link."' ".$targetInsert." class='".$this->stylePrefix."title'>".
				$this->iTrunc(htmlspecialchars($this->title),100)."</a></div>";
		}
			...
			if ($this->items[$i]->title) {
				if ($this->items[$i]->link) {
					$feedArray[] =
						...
						"item_title'".$targetInsert.">".$this->iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)),100).
						"</a></div>";
				} else {
					$feedArray[] =
						"<div class='".$this->stylePrefix."item_title'>".
						$this->iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)),100).
						"</div>";
				}
			}
