<?php
/**
* Jomres CMS Agnostic Plugin
* @author Woollyinwales IT <sales@jomres.net>
* @version Jomres 9 
* @package Jomres
* @copyright	2005-2015 Woollyinwales IT
* Jomres (tm) PHP files are released under both MIT and GPL2 licenses. This means that you can choose the license that best suits your project.
**/

// ################################################################
defined( '_JOMRES_INITCHECK' ) or die( 'Direct Access to this file is not allowed.' );
// ################################################################

class j06001occupancies_save {
	function __construct()
		{
		// Must be in all minicomponents. Minicomponents with templates that can contain editable text should run $this->template_touch() else just return 
		$MiniComponents =jomres_getSingleton('mcHandler');
		if ($MiniComponents->template_touch)
			{
			$this->template_touchable=false; return;
			}
			
		$room_type_id		= intval(jomresGetParam( $_REQUEST, 'room_type_id', 0 ));
		$quantity		= jomresGetParam( $_REQUEST, 'quantity', array() );
		$defaultProperty=getDefaultProperty();
		
		jr_import('jomres_occupancies');
		$occupancies = new jomres_occupancies();
		$occupancies->property_uid = $defaultProperty;
		$occupancies->room_type_id = $room_type_id;
		$occupancies->guest_type_map = $quantity;
		
		$occupancies->delete();
		$occupancies->commit();

		jomresRedirect( jomresURL(JOMRES_SITEPAGE_URL."&task=occupancies_list"), $saveMessage );
		}

	
	// This must be included in every Event/Mini-component
	function getRetVals()
		{
		return null;
		}
	}
