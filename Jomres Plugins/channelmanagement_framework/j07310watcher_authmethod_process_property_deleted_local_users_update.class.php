...
class j07310watcher_authmethod_process_property_deleted_local_users_update
{	
	/**
	 *
	 * Constructor
	...
	public function __construct($componentArgs)
	{
		...
		logging::log_message("CMF property deletion remove property to local users channels : Received instant message with contents " , 'CMF', 'DEBUG' , serialize( $webhook_messages ) );
		...
		if (!empty($webhook_messages)) {
			...
			foreach ( $webhook_messages as $webhook_notification ) {
				if ($webhook_notification->data->manager_id != 0 ) {
					...
					// first we'll remove the property for this manager from the xref table
					if (isset($data) && $data !== false && isset($webhook_notification->webhook_event) && $webhook_notification->webhook_event == 'property_deleted' ) {
						...
						channelmanagement_framework_utilities :: remove_local_properties_of_manager ( $webhook_notification->data->manager_id , $channel_id , array( $webhook_notification->data->property_uid ) );

						// Next we need to find the super property managers and remove the records in the xref table.
						if (!empty($super_property_managers)) {
							foreach ($super_property_managers as $super_manager) {
								if ($webhook_notification->data->manager_id != $super_manager){
									...
									$channel_id = (int)doSelectSql($query,1);
									if ($channel_id > 0) {
										channelmanagement_framework_utilities :: remove_local_properties_of_manager ( $super_manager , $channel_id , array( $webhook_notification->data->property_uid ) );
									}
								}
							}
						}
